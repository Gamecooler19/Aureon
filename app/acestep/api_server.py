...
from acestep.generation_info import _build_generation_info
...
def create_app() -> FastAPI:
    ...
    # Allow all origins so the standalone HTML UI can reach the API from any host
    app.add_middleware(
        CORSMiddleware,
        allow_origins=["*"],
    ...
    # =========================================================================
    # Studio UI — serve studio.html at the root so the browser can reach the
    # full-stack app at http://localhost:3297 (or any configured port).
    # =========================================================================

    _STUDIO_PATH = os.path.join(_get_project_root(), "ui", "studio.html")
    ...
    async def serve_studio():
        ...
        return JSONResponse(
            {"error": "studio.html not found at ui/studio.html"},
            status_code=404,
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--port",
        type=int,
        default=int(os.getenv("ACESTEP_API_PORT", "3297")),
        help="Bind port (default from ACESTEP_API_PORT or 3297)",
    )
    parser.add_argument(
        "--api-key",
        type=str,
